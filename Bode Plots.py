...
# Perturbation transfer function: load torque to angular speed
Gl= -(Re + Le*s)/( Le*J*s**2  + (Re*J+Le*Bv)*s + 2*kv*kt + Re*Bv  )
...
mag, phase, omega = bode_plot(Gc, dB=True, Hz=True, omega_limits=(10, 1e4), \
                              omega_num=1000 )
i=601
print(omega[i]/2/pi, 20*log10(mag[i]), phase[i]*180/pi)

mag, phase, omega = bode_plot(Gl, dB=True, Hz=True, omega_limits=(10, 1e4), \
                              omega_num=100 )
i=0
print(omega[i]/2/pi, 20*log10(mag[i]), phase[i]*180/pi)
